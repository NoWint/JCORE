...
<figure id="fig:intro" data-latex-placement="hbp">
<embed src="/figures/qwen25-technical-report/figures/main.pdf" style="width:92.0%" />
<figcaption>In the iterative development of the Qwen series, data scaling has played a crucial role. Qwen 2.5, which leverages 18 trillion tokens for pre-training, has demonstrated the most advanced capabilities within the Qwen series, especially in terms of domain expertise, underscoring the importance of scale together with mixture in enhancing the model’s capabilities.</figcaption>
</figure>
...
For dense models, we maintain the Transformer-based decoder architecture [@transformer; @gpt] as Qwen2 [@qwen2]. The architecture incorporates several key components: Grouped Query Attention (GQA, [@gqa]) for efficient KV cache utilization, SwiGLU activation function [@glu] for non-linear activation, Rotary Positional Embeddings (RoPE, [@rope]) for encoding position information, QKV bias [@qkv_bias] in the attention mechanism and RMSNorm [@rmsnorm] with pre-normalization to ensure stable training.
...
Building on these techniques, we have developed a larger and higher-quality pre-training dataset, expanding from the 7 trillion tokens used in Qwen2 [@qwen2] to **18 trillion** tokens.
...
To enhance our models' ability to process longer sequences during inference, we implement two key strategies: YARN [@yarn] and Dual Chunk Attention (DCA, [@chunkllama]). Through these innovations, we achieve a four-fold increase in sequence length capacity, enabling Qwen2.5-Turbo to handle up to **1 million** tokens and other models to process up to 131,072 tokens. Notably, these approaches not only improve the modeling of long sequences by reducing perplexity but also maintain the models' strong performance on shorter sequences, ensuring consistent quality across varying input lengths.

# Post-training {#sec:post}
...
1.  **Long-sequence Generation:** Qwen2.5 is capable of generating high-quality content with an output context length of up to 8,192 tokens, a significant advancement over the typical post-training response length, which often remains under 2,000 tokens. To address this gap, we develop long-response datasets [@quan2024language]. We employ back-translation techniques to generate queries for long-text data from pre-training corpora, impose output length constraints, and use Qwen2 to filter out low-quality paired data.

2.  **Mathematics:** We introduce the chain-of-thought data of Qwen2.5-Math [@qwen2.5math], which encompasses a diverse range of query sources, including public datasets, K-12 problem collections, and synthetic problems. To ensure high-quality reasoning, we employ rejection sampling [@yuan2023scaling] along with reward modeling and annotated answers for guidance, producing step-by-step reasoning process.

3.  **Coding:** To enhance coding capabilities, we incorporate the instruction tuning data of Qwen2.5-Coder [@qwen2.5coder]. We use multiple language-specific agents into a collaborative framework, generating diverse and high-quality instruction pairs across nearly 40 programming languages. We expand our instruction dataset by synthesizing new examples from code-related Q&A websites and gathering algorithmic code snippets from GitHub. A comprehensive multilingual sandbox is used to perform static code checking and validate code snippets through automated unit testing, ensuring code quality and correctness [@dou2024multi; @yang2024evaluating].
...
MMLU [@mmlu] (5-shot), MMLU-Pro [@mmlupro] (5-shot), MMLU-redux [@mmluredux] (5-shot), BBH [@bbh] (3-shot), ARC-C [@arc] (25-shot), TruthfulQA [@truthfulqa] (0-shot), Winogrande [@winogrande] (5-shot), HellaSwag [@hellaswag] (10-shot).

#### Mathematics & Science Tasks

GPQA [@gpqa] (5-shot), Theorem QA [@theoremqa] (5-shot), GSM8K [@gsm8k] (4-shot), MATH [@math] (4-shot).

#### Coding Tasks

HumanEval [@humaneval] (0-shot), HumanEval+ [@evalplus](0-shot), MBPP [@mbpp] (0-shot), MBPP+ [@evalplus] (0-shot), MultiPL-E [@multiple] (0-shot) (Python, C++, JAVA, PHP, TypeScript, C#, Bash, JavaScript).

#### Multilingual Tasks

We group them into four categories: (a) Exam: M3Exam (5-shot, we only choose examples that require no image), IndoMMLU [@koto-etal-2023-indommlu] (3-shot), ruMMLU [@rummlu-mera] (5-shot), and translated MMLU [@msift] (5-shot on Arabic, Spanish, French, Portuguese, German, Italian, Japanese, and Korean); (b) Understanding: BELEBELE [@belebele] (5-shot), XCOPA [@xcopa] (5-shot), XWinograd [@xwinograd] (5-shot), XStoryCloze [@xstory_cloze] (0-shot) and PAWS-X [@paws-x] (5-shot); (c) Mathematics: MGSM [@flores] (8-shot CoT); and (d) Translation: Flores-101 [@flores] (5-shot).
...
: **Performance of the 70B+ base models and Qwen2.5-Plus.** {#tab:base-70B}
...
: **Performance of the 14B-30B+ base models and Qwen2.5-Turbo.** {#tab:base-14B}
...
: **Performance of the 7B+ base models.** {#tab:base-7B}

#### Qwen2.5-14B/32B & Qwen2.5-Turbo

The evaluation of the Qwen2.5-Turbo, Qwen2.5-14B, and 32B models is compared against baselines of similar sizes. These baselines include Yi-1.5-34B [@yi], Gemma2-27B [@gemma2], and Qwen1.5-32B [@qwen1.5]. The results are shown in Table [2](#tab:base-14B){reference-type="ref" reference="tab:base-14B"}. The Qwen2.5-14B model demonstrates a solid performance across various tasks, particularly excelling in general tasks like MMLU and BBH, where it achieves scores of 79.7 and 78.2, outcompeting competitors of larger sizes. Meanwhile, Qwen2.5-32B, in particular, showcases exceptional capabilities, often surpassing larger models of similar model sizes. Notably, it outperforms its predecessor Qwen1.5-32B significantly, especially in challenging areas such as mathematics and coding, with notable scores of 57.7 in MATH and 84.5 in MBPP. For Qwen2.5-Turbo, although its training cost and inference cost are significantly smaller than those of Qwen2.5-14B, it achieves comparable results, where its MMLU-Pro score is even better than that of Qwen2.5-32B.

#### Qwen2.5-7B

For 7B-level models, we focus on comparing Qwen2.5-7B with other leading 7B+ models, including Mistral-7B [@mistral], Llama3-8B [@llama3], Gemma2-9B [@gemma2], and our predecessor, Qwen2-7B [@qwen2]. The results can be found in Table [3](#tab:base-7B){reference-type="ref" reference="tab:base-7B"}. Note that the non-embedding parameters of Qwen2-7B and Qwen2.5-7B are only 6.5B, while that of Gemma2-9B is 8.2B. The Qwen2.5-7B model surpasses its predecessors and counterparts in numerous benchmarks, despite having fewer non-embedding parameters. It demonstrates significant improvements across various tasks, achieving 74.2 on general benchmarks like MMLU [@mmlu], 49.8 on math challenges such as MATH [@math], and 57.9 on coding tasks like HumanEval [@humaneval].
...
: **Performance of the smaller base models.** {#tab:base-small}

#### Qwen2.5-0.5B/1.5B/3B

For edge-side models, we compare Qwen2.5-0.5B, 1.5B, and 3B against established baselines: Qwen2-0.5B/1.5B [@qwen2] and Gemma2-2.6B [@gemma2]. The results are given in Table [4](#tab:base-small){reference-type="ref" reference="tab:base-small"}. Qwen2.5-0.5B, 1.5B, and 3B continue to maintain strong performance across nearly all benchmarks. Notably, the Qwen2.5-0.5B model outperforms the Gemma2-2.6B on various math and coding tasks.
...
: **Performance of the 70B+ Instruct models and Qwen2.5-Plus.** {#tab:70b_instruct}
...
: **Performance of the 7B+ instruction-tuned models.** {#tab:7b-instruct}
...
: **Performance comparison of 2B-4B instruction-tuned models.** {#tab:4b-instruct}

#### Other Instruction-tuned Models

As illustrated in Table [7](#tab:7b-instruct){reference-type="ref" reference="tab:7b-instruct"}, the Qwen2.5-7B-Instruct model significantly outperforms its competitors, Gemma2-9B-IT and Llama3.1-8B-Instruct, across all tasks except IFEval. Notably, Qwen2.5-7B-Instruct exhibits clear advantages in mathematics (MATH: 75.5) and coding (HumanEval: 84.8). For the edge-side instruction models, the Qwen2.5-3B-Instruct model, despite having fewer parameters than both the Phi3.5-mini-instruct [@phi3] and MiniCPM3-4B-Instruct [@minicpm] models, surpasses them in mathematics and coding tasks, as shown in Table [8](#tab:4b-instruct){reference-type="ref" reference="tab:4b-instruct"}. Additionally, it delivers competitive results in language understanding. The Qwen2.5-1.5B-Instruct and Qwen2.5-0.5B-Instruct models have also seen substantial performance improvements over their previous versions, as detailed in Table [9](#tab:2b-instruct){reference-type="ref" reference="tab:2b-instruct"}. These enhancements make them particularly well-suited for edge-side applications in highly resource-constrained environments.
...
: **Performance comparison of 0.5B-1.5B instruction-tuned models.** {#tab:2b-instruct}
...
: **Performance Comparison on our in-house English automatic evaluation benchmark.** {#tab:eval_v11_EN}
...
: **Performance Comparison on our in-house Chinese automatic evaluation benchmark.** {#tab:eval_v11_ZH}

Despite the availability of several open benchmark datasets for evaluation, we believe that these are insufficient to fully capture the capabilities of LLMs. To address this, we have developed a series of in-house datasets designed to assess various aspects of model performance, including knowledge understanding, text generation, coding, and more. These evaluations are conducted in both Chinese and English. In addition, we have specifically evaluated the multilingual performance of instruction-tuned models. The results are summarized in Table [10](#tab:eval_v11_EN){reference-type="ref" reference="tab:eval_v11_EN"} for English, Table [11](#tab:eval_v11_ZH){reference-type="ref" reference="tab:eval_v11_ZH"} for Chinese, Table [12](#tab:eval_multilingual_70B){reference-type="ref" reference="tab:eval_multilingual_70B"} for multilingualism of 70B+ Instruct models, and Table [13](#tab:eval_multilingual_14B){reference-type="ref" reference="tab:eval_multilingual_14B"} for 7B-14B models, respectively.
...
: **Performance of the 70B+ Instruct models on Multilingual Tasks.** {#tab:eval_multilingual_70B}
...
: **Performance comparison across multiple RM benchmarks.** {#tab:eval_reward_bench_metrics}
...
|                                       | **RULER**                                                                                                                                                         |                             |                             |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| 3-9                                   |             | **Avg.**                    | **4K**                      | **8K**                      | **16K**                     | **32K**                     | **64K**                     | **128K**                    |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
...
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| [w/o DCA + YARN]{style="color: gray"} |             | [80.1]{style="color: gray"} | [96.7]{style="color: gray"} | [95.1]{style="color: gray"} | [93.7]{style="color: gray"} | [89.4]{style="color: gray"} | [74.5]{style="color: gray"} | [31.4]{style="color: gray"} |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| **Qwen2.5-14B-Instruct**              | 128K        | 91.4                        | 97.7                        | 96.8                        | 95.9                        | 93.4                        | 86.7                        | 78.1                        |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| [w/o DCA + YARN]{style="color: gray"} |             | [86.5]{style="color: gray"} | [97.7]{style="color: gray"} | [96.8]{style="color: gray"} | [95.9]{style="color: gray"} | [93.4]{style="color: gray"} | [82.3]{style="color: gray"} | [53.0]{style="color: gray"} |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| **Qwen2.5-32B-Instruct**              | 128K        | 92.9                        | 96.9                        | 97.1                        | 95.5                        | 95.5                        | 90.3                        | 82.0                        |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| [w/o DCA + YARN]{style="color: gray"} |             | [88.0]{style="color: gray"} | [96.9]{style="color: gray"} | [97.1]{style="color: gray"} | [95.5]{style="color: gray"} | [95.5]{style="color: gray"} | [85.3]{style="color: gray"} | [57.7]{style="color: gray"} |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| **Qwen2.5-72B-Instruct**              | 128K        | **95.1**                    | **97.7**                    | **97.2**                    | **97.7**                    | **96.5**                    | **93.0**                    | **88.4**                    |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| [w/o DCA + YARN]{style="color: gray"} |             | [90.8]{style="color: gray"} | [97.7]{style="color: gray"} | [97.2]{style="color: gray"} | [97.7]{style="color: gray"} | [96.5]{style="color: gray"} | [88.5]{style="color: gray"} | [67.0]{style="color: gray"} |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
| **Qwen2.5-Turbo**                     | 1M          | 93.1                        | 97.5                        | 95.7                        | 95.5                        | 94.8                        | 90.8                        | 84.5                        |
+---------------------------------------+-------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+

: **Performance of Qwen2.5 Models on RULER.** *YARN+DCA* does not change the model behavior within 32K tokens. {#tab:ruler}
...
[w/o DCA + YARN]{style="color: gray"} & & [55.9]{style="color: gray"}&[49.7]{style="color: gray"}&[33.1]{style="color: gray"}&[13.6]{style="color: gray"}&[0.5]{style="color: gray"} & [-]{style="color: gray"}\
**Qwen2.5-14B-Instruct** & 128k & 53.0 &50.8 & 46.8 & 43.6 & 39.4 & 8.04\
[w/o DCA + YARN]{style="color: gray"}& & [53.0]{style="color: gray"} &[50.8]{style="color: gray"} & [37.0]{style="color: gray"} & [18.4]{style="color: gray"} & [0.8]{style="color: gray"} & [-]{style="color: gray"}\
**Qwen2.5-32B-Instruct** & 128k& 56.0 & 53.6 & 48.8 & 45.3 & 41.0 & 8.70\
[w/o DCA + YARN]{style="color: gray"} & & [56.0]{style="color: gray"} & [53.6]{style="color: gray"} & [40.1]{style="color: gray"} & [20.5]{style="color: gray"} & [0.7]{style="color: gray"} & [-]{style="color: gray"}\
**Qwen2.5-72B-Instruct** & 128k & **60.4 & **57.5 & **53.9 & **50.9 & **45.2 & **8.72\
[w/o DCA + YARN]{style="color: gray"} & & [60.4]{style="color: gray"} & [57.5]{style="color: gray"} & [47.4]{style="color: gray"} & [27.0]{style="color: gray"} & [2.4]{style="color: gray"} & [-]{style="color: gray"}\
**Qwen2.5-Turbo** & 1M & 53.4 & 50.0 &45.4 & 43.9 & 38.0& 8.34\**************************
:::

Furthermore, as shown in Figure [2](#fig:passkey_retrieval){reference-type="ref" reference="fig:passkey_retrieval"}, Qwen2.5-Turbo achieves 100% accuracy in the 1M-token passkey retrieval task, demonstrating its exceptional ability to capture detailed information from ultra-long contexts. We develop a sparse attention mechanism based on Minference [@jiang2024minference] to significantly enhance inference speed, which is critical for user experience when processing long contexts. For sequences of 1M tokens, this approach reduces the computational load of the attention mechanism by 12.5 times. Figure [3](#fig:inference_time){reference-type="ref" reference="fig:inference_time"} illustrates the time to first token (TTFT) of Qwen2.5-Turbo across various hardware configurations, where our method achieves a 3.2 to 4.3 times speedup.

<figure id="fig:passkey_retrieval" data-latex-placement="tbp">
<embed src="/figures/qwen25-technical-report/figures/passkey_retrieval.pdf" />
<figcaption><strong>Performance of Qwen2.5-Turbo on Passkey Retrieval Task with 1M Token Lengths.</strong></figcaption>
</figure>

<figure id="fig:inference_time" data-latex-placement="tb">
<embed src="/figures/qwen25-technical-report/figures/inference_speed.pdf" style="width:80.0%" />
<figcaption><strong>TTFT (Time To First Token) of Qwen2.5-Turbo and Qwen2.5-7B with Full Attention and Our Method.</strong></figcaption>
</figure>
...
[^1]: Qwen2.5-Turbo is identified as `qwen-turbo-2024-11-01` and Qwen2.5-Plus is identified as `qwen-plus-2024-xx-xx` (to be released) in the API.

[^2]: For simplicity, we report the results of the subset *strict-prompt*.
